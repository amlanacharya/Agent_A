...
**Future external integrations:** an alternative `PlotEngine` implementation (a remote plotting microservice, a Jupyter rendering kernel, a third-party charting library) can plug in behind the same `PlotEngine` interface without changing the FastAPI surface. The cockpit UI and the nine surface routers stay unchanged. Today's deployment uses the in-process implementation; the seam exists for the day the team chooses to add a real external rendering service, not because one is planned. **Phase 9 (Cockpit Frontend) ships the web frontend that consumes this FastAPI surface in-repo** — the frontend is part of the platform's own native layer, not an external integration.

### Phase 9: Cockpit Frontend

The platform's own native layer for: (1) a typed, design-system-driven React SPA that consumes the FastAPI surface shipped in Phase 8, (2) a token-level port of `prototype/data_intelligence_cockpit/DESIGN.md` into Tailwind config + base components (replacing the throwaway HTML mockups), and (3) a one-surface-per-CB rollout that mirrors the Phase 8 backend cadence. The frontend is the surface the human planner interacts with; the FastAPI surface (Phase 8) is the engine. The boundary is deliberately kept behind small seams: (a) the `openapi-typescript`-generated types in `frontend/src/api/schema.ts` are the single source of truth for the contract, (b) TanStack Query is the single source of truth for cache/lifecycle, (c) the `<PlotFrame>` wrapper is the single source of truth for plot rendering. A future alternative frontend (a Svelte port, a mobile-native shell, a Jupyter widget) can plug in behind the same FastAPI surface without changing the backend.

**What this repo ships (Phase 9 in-repo):**

- [x] CB1 — rewrite the Phase 9 section in this plan to make the in-repo / out-of-repo split explicit; document the never-commit paths (`frontend/dist/`, `frontend/node_modules/`, `frontend/src/api/schema.ts`); add the frontend to the standing-rules handoff doc (done 2026-06-18, `f18edc4`).
- [x] CB2 — frontend scaffold + DESIGN.md tokens (done 2026-06-18, `fb14dd2`): `frontend/` package with Vite + React 18 + TS + Tailwind 3 + TanStack Query 5 + Recharts + Vitest + RTL. Tailwind theme extends DESIGN.md tokens (Deep Indigo #3525cd primary, Electric Cyan #4cd7f6 accent, Light Slate #f8fafc canvas, Inter + JetBrains Mono). 7 tests passing (5 tokens unit + 2 App scaffold). `pnpm build` produces a working `frontend/dist/` bundle. `uv run pytest -q` still at 846.
- [x] CB3 — API client + openapi-typescript codegen + contract test (done 2026-06-18, `f8934a5`): `openapi-typescript` reads the live FastAPI `/openapi.json` and writes `src/api/schema.ts` (gitignored, regenerated). `src/api/client.ts` exports typed `listSurfaces()`, `fetchSurface(name, runId)`, `renderPlot(request)` over a `/api` base (Vite proxies to `http://localhost:8000` in dev; `setApiBase()` overrides in tests). `src/api/hooks.ts` wraps them in TanStack Query (`useSurfaces`, `useSurface`). `frontend/tests/contract.test.ts` spawns `uv run python -m api` once and exercises all 9 surfaces + all 7 plot kinds against real responses — 18 contract tests green. New backend entry: `api/__main__.py` (a dev launcher that wires all 10 surfaces with canned fixtures). `pnpm test` total: 25 tests (5 tokens + 2 App + 18 contract). `uv run pytest -q` unchanged at 846.
- [x] CB4 — routing + global layout + GET /runs backend extension (done 2026-06-18, `de978c2`): React Router v6 with `RouterProvider`, `createBrowserRouter` mapping `/` → Mission Control, `/surfaces/:name/:runId` → `<GenericSurfacePage>`. New components in `frontend/src/components/`: `<PageHeader>` (eyebrow + title + subtitle + actions), `<StatusChip>` (success/warning/critical/info/neutral tones via DESIGN.md semantic palette), `<MetricCard>` (white card + 24px padding + optional 2px cyan top accent for active metrics), `<AppShell>` (top nav with brand mark + surface menu + run selector, reads `useSurfaces()`). New backend endpoint `GET /runs` in `api/app.py` lists run directories under `outputs/` containing any Phase 7 monitoring artifact (scratch dirs filtered out). Two backend tests added (filter behavior + missing-root edge case). 35 frontend tests total (4 App router + 3 PageHeader + 3 StatusChip + 3 MetricCard + 5 tokens + 18 contract). `uv run pytest -q` at 848 (was 846).
- [x] CB5 — Mission Control surface (done 2026-06-18, `b08c3a5`): bespoke `frontend/src/pages/MissionControl.tsx` wired into the router under `/surfaces/mission_control/:runId`. Reads `useSurface('mission_control', runId)` and renders 3 zones: KPI grid (current step + active agent + confidence + blockers, with success/warning/critical StatusChip on confidence), approval banner (warning tone + verifier gate + attempt counter), activity feed (tool_result + code_escalation_status + blockers with severity-coded chips). `MissionControlState` interface matches the FastAPI `CockpitState.to_public_dict()` shape (current_step, active_agent, tool_result, code_escalation_status, code_attempt, verifier_gate, approval_needed, confidence, blockers) — no contract changes. 5 MissionControl tests added (KPI render, approval banner show/hide, activity feed, confidence tone).
- [x] CB6 — Data Health surface (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/DataHealth.tsx` wired at `/surfaces/data_health/:runId`. New `<DataTable>` base component (flat table per DESIGN.md: horizontal-only borders, row hover = #f8fafc, label-caps headers, optional numeric/mono columns). Page renders 3 KPI cards (series, segments, demand classes) + segment-profiles table (segment_id, series_count, median_adi, median_cv2, demand-class breakdown, forecastability breakdown) + narrative card. Reads `useSurface('data_health', runId)`. 4 DataHealth tests added.
- [x] CB7 — EDA Explorer surface (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/EdaExplorer.tsx` wired at `/surfaces/eda_explorer/:runId`. New `<PlotFrame>` wrapper (single-source consumer of the FastAPI /plots endpoint — renders base64 PNG/SVG as <img>, 5-min staleTime on heavy bytes) and `<FilterPanel>` (3-column select row: series + granularity + date range). Page renders 3 KPI cards + 3 PlotFrames (demand_curve, sparsity, anomalies) + per-series profiles DataTable with demand-class StatusChips. 3 EdaExplorer tests added.
- [x] CB8 — Feature Factory surface (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/FeatureFactory.tsx` wired at `/surfaces/feature_factory/:runId`. Renders 4 KPI cards (Series count + 3 most-enabled families) + per-series families grid (8 family booleans per series as ON/off StatusChips) + recommended-models DataTable + feature_importance PlotFrame. State shape: dict[series_key, FeatureFlags.model_dump()] + optional recommended_models_per_series. 3 FeatureFactory tests.
- [x] CB9 — Model Arena surface (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/ModelArena.tsx` wired at `/surfaces/model_arena/:runId`. Renders 3 KPI cards (scorecard count, model families, best MASE with PASS/REVIEW chip) + leaderboard DataTable (model × series × fold with MASE / MAE / RMSE / bias) + frequently-promoted / never-surfaced chip clusters + ensemble_weights JSON dump + forecast_band PlotFrame. 4 ModelArena tests.
- [x] CB10 — Forecast Review surface (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/ForecastReview.tsx` wired at `/surfaces/forecast_review/:runId`. Renders 3 KPI cards (overall MASE with PASS/REVIEW chip, target-met fraction, series result count) + per-series results DataTable with MET/MISS chips + forecast_band + backtest PlotFrames + foundry narrative. 4 ForecastReview tests.
- [x] CB11 — Replenishment Board + Learning Journal (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/ReplenishmentBoard.tsx` (4 KPI cards: recommendation count + total order qty + high-tier count + medium/low tier counts; recommendations table with tier StatusChips) wired at `/surfaces/replenishment_board/:runId`; bespoke `frontend/src/pages/LearningJournal.tsx` (3 KPI cards: active cards + retired cards + artifact count; 6-tab workspace markdown viewer for LEARNINGS / DECISIONS / ASSUMPTIONS / RUNBOOK / MODEL_REGISTRY / PROMOTION_DECISIONS) wired at `/surfaces/learning_journal/:runId`. 2 + 3 tests.
- [x] CB12 — MLOps Monitor + `<PlotFrame>` wrapper + e2e + delete `prototype/` + glossary (done 2026-06-18, `<sha>`): bespoke `frontend/src/pages/MlopsMonitor.tsx` wired at `/surfaces/mlops_monitor/:runId` (4 artifact KPI cards + tabbed markdown viewer for MONITORING_REPORT / DRIFT_REPORT / OVERRIDE_ANALYSIS / MODEL_HEALTH). `<PlotFrame>` wrapper absorbs all 7 plot kinds from CB7/CB10 into a single reusable component. New e2e test (`frontend/tests/e2e.test.ts`) spawns `uv run python -m api` in beforeAll and walks every surface + `/surfaces/{name}/{run_id}` + `/runs` against real FastAPI responses. 2 MlopsMonitor tests + 12 e2e tests added. `api/__main__.py` updated to pass `outputs_root` (was defaulting to cwd-relative `./outputs` which broke `/runs`). CONTEXT.MD gained a "Cockpit (Phase 9 frontend)" section with `Cockpit Frontend`, `Plot Frame`, `Surface Route` glossary terms. `prototype/` deleted (11 subfolders; patterns absorbed into `frontend/src/components/` + `frontend/src/pages/`). Final test counts: 77 frontend tests (component + contract + e2e), 848 backend tests. `pnpm build` clean (12 kB CSS + 284 kB JS bundle). Phase 9 complete.

**What this repo does NOT ship (Phase 9 out-of-repo):**
...
- **Frontend never-commit paths**: `frontend/dist/`, `frontend/node_modules/`, `frontend/src/api/schema.ts` (regenerated by `pnpm generate-api`). The SPA build artifact is mounted by `api/app.py`'s `StaticFiles`, not committed to git.
...
## Progress Summary (as of 2026-06-18)
